...
def _side_stats(takeovers, entity, other_entity) -> dict:
    stats = takeovers.filter(represented_entity_id=entity.id).aggregate(
        takeovers=Count("id"),
    ...
    stats["accent_color"] = (
        entity.accent_color
        if HEX_COLOR_PATTERN.fullmatch(entity.accent_color)
        else "#b3262f"
    )
    ...
    stats["attacks"] = takeovers.filter(
        represented_entity_id=entity.id,
        board__entity_id=other_entity.id,
    ).count()
    return stats
...
def build_rivalry_scoreboard(rivalry: Rivalry, period: str = "all") -> dict:
    ...
    entity_a = rivalry.entity_a
    entity_b = rivalry.entity_b
    side_a = _side_stats(takeovers, entity_a, entity_b)
    side_b = _side_stats(takeovers, entity_b, entity_a)

    if side_a["takeovers"] > side_b["takeovers"]:
        leader = side_a
        lead_text = f"{entity_a.name} leads by {side_a['takeovers'] - side_b['takeovers']} takeover wins."
    elif side_b["takeovers"] > side_a["takeovers"]:
        leader = side_b
        lead_text = f"{entity_b.name} leads by {side_b['takeovers'] - side_a['takeovers']} takeover wins."
    elif side_a["spend_cents"] > side_b["spend_cents"]:
        leader = side_a
        lead_text = f"{entity_a.name} leads on backing after a tie in takeover wins."
    elif side_b["spend_cents"] > side_a["spend_cents"]:
        leader = side_b
        lead_text = f"{entity_b.name} leads on backing after a tie in takeover wins."
    else:
        leader = None
        lead_text = "Dead even."

    boards = list(
        Board.objects.filter(entity_id__in=[entity_a.id, entity_b.id])
        .select_related("entity", "current_controller", "current_bid__represented_entity")
        .order_by("entity__name")
    )
    for board in boards:
        board.safe_accent_color = (
            board.entity.accent_color
            if HEX_COLOR_PATTERN.fullmatch(board.entity.accent_color)
            else "#b3262f"
        )
    recent_moves = list(
    ...
    biggest_move = takeovers.select_related("board__entity", "represented_entity").order_by(
        "-amount_cents", "-occurred_at", "-id"
    ).first()

    leader_entity_id = leader["entity"].id if leader else None
