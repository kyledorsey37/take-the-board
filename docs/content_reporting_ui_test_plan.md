...
**Scope:** Public reporting, moderator review, Stripe capture snapshots, and
fee-deducted refunds for Take the Board.
...
3. For Stripe scenarios, use a Stripe CLI webhook forwarder configured with the
   local webhook secret. Verify that `payment_intent.amount_capturable_updated`,
   `payment_intent.succeeded`, and `charge.updated` events reach the app.
...
5. Keep a note of the school board and the public takeover/bid IDs for the test
...
### 7. Capture snapshot appears in Admin
...
### 8. Remove a captured paid message: refund less the actual fee
...
### 9. Fee-not-yet-available safety check
...
- Once the fee data becomes available, the worker calculates the net amount and
  completes the normal refund flow in scenario 8.

## Operational checks

### 10. One-time historical snapshot backfill
