...
| 2026-09-03 | Dev PostgreSQL env split | Analytics, SEO, accessibility, and public product behavior are N/A. Dedicated PostgreSQL env-file enforcement, `0600` permissions, legacy `.env` migration, secret-safe deployment reporting, and unchanged named-volume behavior are covered by focused contract tests and deployment docs. Remaining gates are host execution, persistence verification for existing dev users/content, and external launch checks. |
...
| 2026-09-03 | Board-level X sharing | Added a separate server-encoded X post intent beside the existing native-share/clipboard control. Reused `board_share_clicked` and `board_share_result` with low-cardinality parameters only; live X card preview validation remains a production-domain launch follow-up. |
...
| 2026-09-03 | Moderation trial leniency | Raised message and display-name moderation headroom and changed rejection backoff to begin after eight failures with a 30-second start and five-minute cap. Retained the 30 uncached Bedrock calls/minute global cap, IP/candidate quotas, concurrency semaphore, and circuit breaker. No new GA4 markers. |
| 2026-09-03 | Supply-chain enforcement | Added a digest-pinned Gitleaks pre-commit hook and read-only GitHub Actions secret/dependency checks. The local hook protects VS Code and terminal commits; requiring the workflow status check, enabling hosted dependency alerts, and running the full Git-history scan remain operator gates. |
| 2026-09-03 | Local HTMX asset delivery | Vendored the exact HTMX 1.9.12 release with provenance, license, and checksum; removed unpkg from the report-only CSP and preserved conditional official Stripe.js plus consent-gated GA. No new analytics markers; public SEO, accessibility, and product behavior are unchanged. Static collection is covered by the existing image build. Added focused asset/CSP/template tests, ran Django checks and the required live board curl, and documented the controlled future HTMX upgrade path. |
| 2026-09-03 | Brand asset package | Added local canonical SVG lockups plus a separately simplified square mark and browser/Apple exports. Header/error-page image treatment is decorative with an accessible “Take the Board” link name; no new analytics interaction was introduced. Existing canonical/escaped OG/X board metadata remains unchanged, the social card was intentionally not graphically redesigned, and live preview cache validation remains a launch follow-up. |
